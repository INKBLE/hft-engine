...
struct LimitOrderBook
{
    ...
    void registerTradeCallback(TradeCallback cb) { on_trade_ = cb; }

    LimitOrderBook(size_t max_orders = 0)
        : bids_pool_(96, 1024), asks_pool_(96, 1024),
          bids_(std::greater<Price>(),
                PoolAllocator<std::pair<const Price, PriceLevel>>(bids_pool_)),
    ...
    struct OrderLocation
    {
        ...
        OrderLocation(Price p, Side s, size_t idx)
            : price(p), side(s), index(idx), valid(true)
        {
        }
    };
    ...
    void addOrder(Order order)
    {
        if (order.side == Side::BUY) // 是买单
        {
            ...
            if (!asks_.empty())
            {
                ...
                while (it != asks_.end() && it->first <= order.price)
                {
                    ...
                    if (cur_price_level.total_qty != 0)
                    {
                        ...
                        for (size_t i = cur_price_level.head_index;
                             i < cur_count; ++i)
                        {
                            if (!cur_price_level.orders[i].is_canceled)
                            {
                                ...
                                if (order.qty >=
                                    cur_price_level.orders[i]
                                        .qty) // 买单需要的比当前找到的卖单多，即没消化完
                                {
                                    matched_qty = cur_price_level.orders[i]
                                                      .qty; // 记录成交量
                                    order.qty -= cur_price_level.orders[i].qty;
                                    cur_price_level.orders[i].is_canceled =
                                        true;
                                    cur_price_level.total_qty -=
                                        cur_price_level.orders[i].qty;
                                    if (cur_price_level.head_index == i)
                                    {
                                        while (cur_price_level.head_index <
                                                   cur_count &&
                                               cur_price_level
                                                   .orders[cur_price_level
                                                               .head_index]
                                                   .is_canceled)
                                        {
            ...
            order_map_[order.id] = {order.price, order.side,
                                    it->second.orders.size() - 1};
        }
        else // 是卖单
        {
            // 撮合循环
                    ...
                    if (cur_price_level.total_qty != 0)
                    {
                             ...
                             i < cur_count; ++i)
                        {
                            if (!cur_price_level.orders[i].is_canceled)
                            {
                                ...
                                // 成交回调：移到 break 之前，确保 partial fill
                                // 也能触发
                                if (on_trade_)
                                {
                                    on_trade_(Trade{
