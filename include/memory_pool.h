...
class MemoryPool
{
  public:
    // 构造池子
    MemoryPool(size_t slot_size, size_t num_slots)
        : slot_size_(slot_size), num_slots_(num_slots)
    {
        ...
        for (size_t i = 0; i < num_slots_ - 1; ++i)
        {
            ...
            reinterpret_cast<FreelistNode*>(cur_slot)->next =
                reinterpret_cast<FreelistNode*>(next_slot);
        }
    ...
    // 释放池子
    ~MemoryPool() { delete[] pool_memory_; }
