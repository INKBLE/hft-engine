...
int main()
{
    // std::cout << "map node size: " << sizeof(std::map<hft::Price,
    // hft::PriceLevel>::value_type) << std::endl; std::cout << "map node
    // alignment: " << alignof(std::map<hft::Price,
    // hft::PriceLevel>::value_type) << std::endl;

    hft::LimitOrderBook book(2000000);

    // 1. 注册成交回调函数（在这里打印成交回报）
    book.registerTradeCallback(
        [](const hft::Trade& trade)
        {
            std::cout << "【成交回报】 "
                      << "价格: " << trade.price << ", 数量: " << trade.qty
                      << ", 买单ID: " << trade.buy_order_id
                      << ", 卖单ID: " << trade.sell_order_id << std::endl;
        });
    ...
    std::cout
        << "买单 2 撮合完毕。吃掉卖单 1 的 10 个数量，自身剩余 5 个挂入 bids_。"
        << std::endl;
    ...
    std::cout << "撤单后，该价格档位的 total_qty 总量应为 0："
              << book.bids_.at(100).total_qty << std::endl;
